# Projects

Each subfolder is one printable thing. Convention:

- `README.md` — what it is, recommended print settings, photos later
- source CAD files (`.scad`, `.step`, `.FCStd`)
- exported `.stl` for direct slicing
- optional `.3mf` with known-good Bambu Studio settings baked in

## What's here

| Folder | Description |
|---|---|
| [example-benchy/](example-benchy/) | The canonical printer torture-test; your first "real" print |
| [wife-measuring-cylinder/](wife-measuring-cylinder/) | 6" finger-width ruler cylinder (OpenSCAD source) |
| [community-prints/](community-prints/) | Curated list of free models worth downloading |
